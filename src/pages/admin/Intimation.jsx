...
const INTIMATION_SHEET = import.meta.env.VITE_SHEET_INTIMATION;
  ...
  const fetchData = async () => {
    try {
      ...
      const [installRes, intimationRes] = await Promise.all([
        ...
        fetch(`${API_URL}?sheet=${INTIMATION_SHEET}&action=getData`)
      ]);
      ...
      // Logic: Column V (index 21) != Null AND Column W (index 22) == Null
      if (installData.success && installData.data) {
        ...
        for (let i = 6; i < installData.data.length; i++) {
          ...
          const colV = row[21]; // Column V
          const colW = row[22]; // Column W

          if (colV && String(colV).trim() !== "" && (!colW || String(colW).trim() === "")) {
            pData.push({
              rowIndex: i + 1,
              ...
              nextDate: row[26] || '',        // AA: Next Date
              customerRemarks: row[27] || '', // AB: Customer Remarks
              workingHoursShared: row[25] || '', // Z: Working Hours Shared
      ...
      // Process History Data from Intimation sheet
    ...
    try {
      ...
      historyData.forEach(item => {
        if (item.intimationNo && item.intimationNo.startsWith('IS-')) {
          const numString = item.intimationNo.split('-')[1];
      ...
      // Generate Timestamp (YYYY-MM-DD hh:mm:ss) - same format as Next Date
      const now = new Date();
      ...
      // Prepare Row Data for 'Intimation' sheet
      const intimationRowData = [
        ...
        formData.workingHours,              // F: Working Hours
        formData.followUpStatus === "Done" ? "" : formData.nextDate, // G: Next Date
        formData.remarks                    // H: Remarks
      ];

      // Insert into Intimation Sheet
      const res = await fetch(API_URL, {
        method: 'POST',
        body: new URLSearchParams({
          action: 'insert',
          sheetName: INTIMATION_SHEET,
          rowData: JSON.stringify(intimationRowData)
        })
      });

      const result = await res.json();
      if (result.success) {
        // Update Service Installation sheet Column W to mark as processed
        const updateRes = await fetch(API_URL, {
          method: 'POST',
          body: new URLSearchParams({
            action: 'updateCell',
            sheetName: INSTALLATION_SHEET,
            row: selectedItem.rowIndex,
            column: 23, // Column W (index 22, but 1-indexed = 23)
            value: timestamp
          })
        });
          ...
          <div className="flex-1 overflow-auto [&::-webkit-scrollbar]:hidden [-ms-overflow-style:'none'] [scrollbar-width:'none']">
            {activeTab === 'pending' ? (
              <>
                    ...
                    <tbody className="bg-white divide-y divide-gray-200">
                      {filteredPending.map((item, idx) => (
                        <tr key={idx} className="hover:bg-gray-50 transition-colors">
                          ...
                          <td className="px-4 py-3 whitespace-nowrap text-sm text-sky-600 font-medium">{formatDate(item.nextDate) || '-'}</td>
                ...
                {/* Mobile Card View */}
                <div className="lg:hidden flex-1 overflow-auto p-4 space-y-4 [&::-webkit-scrollbar]:hidden [-ms-overflow-style:'none'] [scrollbar-width:'none']">
                  {filteredPending.map((item, idx) => (
                    <div key={idx} className="bg-white border border-gray-200 rounded-xl p-4 shadow-sm space-y-3">
                      ...
                      <div className="grid grid-cols-2 gap-3">
                        ...
                        <div className="space-y-1">
                          <p className="text-[10px] text-gray-400 uppercase font-bold tracking-tight">Install/Service</p>
                          <p className="text-xs font-semibold text-gray-700">{item.installationService || '-'}</p>
                        </div>
                      </div>
