...
const INTIMATION_SERVICE_2_SHEET = import.meta.env.VITE_SHEET_INTIMATION_SERVICE_2;
...
const IntimationService2Page = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      const [installRes, intimationRes] = await Promise.all([
        ...
        fetch(`${API_URL}?sheet=${INTIMATION_SERVICE_2_SHEET}&action=getData`)
      ]);
      ...
      // Logic: Column AC (index 28) != Null AND Column AD (index 29) == Null
      if (installData.success && installData.data) {
        ...
        for (let i = 6; i < installData.data.length; i++) {
          ...
          const colAC = row[28]; // Column AC
          const colAD = row[29]; // Column AD

          if (colAC && String(colAC).trim() !== "" && (!colAD || String(colAD).trim() === "")) {
            pData.push({
              rowIndex: i + 1,
              ...
              nextDate: row[33] || '',        // AH: Next Date
              customerRemarks: row[34] || '', // AI: Customer Remarks
              workingHoursShared: row[32] || '', // AG: Working Hours Shared
      ...
      // Process History Data from Service Installation 2 sheet
  ...
  const filteredPending = filterData(pendingData);
  const filteredHistory = filterData(historyData);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Prepare row data for Service Installation 2 sheet
      ...
      const rowData = [
        ...
        formData.workingHours,        // F: Working Hours Shared By Customer Per Day
        formData.followUpStatus === "Done" ? "" : formData.nextDate, // G: Next Date (blank if Done)
        formData.remarks              // H: Remarks
      ];

      // Insert into Service Installation 2 sheet
      const response = await fetch(API_URL, {
        method: 'POST',
        body: new URLSearchParams({
          action: 'insert',
          sheetName: INTIMATION_SERVICE_2_SHEET,
...
export default IntimationService2Page;